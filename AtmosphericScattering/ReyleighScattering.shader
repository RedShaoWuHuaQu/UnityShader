Shader "Models/ReyleighScattering"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            bool IntersectionAtmosphere(float3 ori, float3 dir, float3 plCenter, float plRadius, float atmoRadius, out float tEnter, out float tExit)
            {
                ...
                if (withPlanet)
                {
                    ...
                    {
                        tEnter = 0;
                        if (tPlanetEnter > 0.001 && tPlanetExit > 0.001)
                        {
                            //TODO：让天空黑色的罪魁祸首
                            tExit = tPlanetEnter;
                        }
            ...
            float PhaseFunction(float3 rayDir, float3 sunDir)
            {
                ...
                return factor * (1 + dot(-rayDir, sunDir) * dot(-rayDir, sunDir));
            }
            ...
            fixed4 frag (v2f f) : SV_Target
            {
                ...
                float3 toneMapped = 1.0 - exp(-finalColor * _Exposure);
        ...
        Pass
        {
            ...
            fixed4 frag(v2f f) : SV_Target
            {
                float3 mainColor = tex2D(_MainTex, f.uv) * 0.6;
                float3 atmoColor = tex2D(_AtmosphereTex, f.uv) * _AtmoMultiple;
                float3 finalColor = mainColor + atmoColor;
